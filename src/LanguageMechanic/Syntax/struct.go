...
func Struct() {
	...
	alignmentBoundary := unsafe.Alignof(example1)
	fmt.Printf("alignment boundary :%d\n", unsafe.Alignof(Example{ //为什么这个是4呢 我本以为这个值是固定的
		BoolValue:  false,
		IntValue:   0,
		FloatValue: 3.1,
	}))
	...
	sizeBool := unsafe.Sizeof(example1.BoolValue)
	offsetBool := unsafe.Offsetof(example1.BoolValue)

	sizeInt := unsafe.Sizeof(example1.IntValue)
	offsetInt := unsafe.Offsetof(example1.IntValue)

	sizeFloat := unsafe.Sizeof(example1.FloatValue)
	offsetFloat := unsafe.Offsetof(example1.FloatValue)
	...
	fmt.Printf("BoolValue = Size: %d Offset: %d Addr: %v\n",
		sizeBool, offsetBool, &example1.BoolValue)

	fmt.Printf("IntValue = Size: %d Offset: %d Addr: %v\n",
		sizeInt, offsetInt, &example1.IntValue)

	fmt.Printf("FloatValue = Size: %d Offset: %d Addr: %v\n",
		sizeFloat, offsetFloat, &example1.FloatValue)
